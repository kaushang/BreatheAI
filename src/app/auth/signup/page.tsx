...
import { INDIAN_CITIES, findCityByName, findNearestCity } from "@/constants/cities";
...
export default function SignupPage() {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      setIsOAuth(true);
      setError("Success! Now please provide your age to complete your profile.");
    } catch (err: any) {
    ...
    <div>
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground font-[family-name:var(--font-outfit)]">
          {isOAuth && step === 1 ? "Complete your profile" : "Create your account"}
        </h1>
      ...
      {error && (
        <div className={`mb-5 rounded-lg border px-4 py-3 text-sm ${
          error.includes("resent") || error.includes("inbox") || error.includes("Success")
      ...
      <div className="flex items-center gap-3 mt-6">
        ...
        {step < TOTAL_STEPS ? (
          ...
          >
            {loading ? "Processing..." : step === 2 ? "I have verified" : "Continue"}
          </button>
        ) : (
          <button
            type="button"
          ...
          return (
            <div key={label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span className={`text-xs font-medium ${isCurrent ? "text-foreground" : isCompleted ? "text-primary" : "text-muted-foreground"}`}>
                  {label}
                </span>
              </div>
              {stepNum < TOTAL_STEPS && (
                <div className="flex-1 mx-2 mt-[-18px]">
                  <div className={`h-[1.5px] w-full transition-colors ${currentStep > stepNum ? "bg-primary" : "bg-border"}`} />
                </div>
              )}
            </div>
          );
    ...
    <div className="space-y-4">
      {!isOAuth ? (
        <>
          ...
          <div className="relative flex items-center">
            ...
            <span className="px-3 text-[10px] text-muted-foreground font-medium uppercase tracking-wider">or sign up with email</span>
            <div className="flex-1 h-px bg-border" />
          </div>
        </>
      ) : (
        <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20 mb-2">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-primary">Authenticated with Google</p>
            <p className="text-[11px] text-muted-foreground">{formData.email}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2 sm:col-span-1">
          <label htmlFor="signup-fullname" className="block text-sm font-medium text-foreground mb-1.5">Full name</label>
          <input
            id="signup-fullname"
            type="text"
        ...
        <div className="col-span-2 sm:col-span-1">
          <label htmlFor="signup-age" className="block text-sm font-medium text-foreground mb-1.5">Age</label>
          <input
            id="signup-age"
            type="number"
            value={formData.age}
      ...
      {!isOAuth && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="signup-confirm" className="block text-sm font-medium text-foreground mb-1.5">Confirm</label>
              <input
                id="signup-confirm"
                type={showPassword ? "text" : "password"}
          ...
          <div className="flex items-center gap-2">
             <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-xs text-muted-foreground hover:text-primary transition-colors">
    ...
    <div className="space-y-6 text-center py-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-2 max-w-[280px] mx-auto">
          We&apos;ve sent a verification link to <span className="font-semibold text-foreground">{email}</span>. Click the link in your email to continue.
        </p>
      </div>
...
function StepLocation({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative flex items-center">
        <div className="flex-1 h-px bg-border" />
        <span className="px-3 text-[10px] text-muted-foreground font-medium uppercase tracking-wider">or search manually</span>
        <div className="flex-1 h-px bg-border" />
      </div>

      <div ref={dropdownRef} className="relative">
        <label htmlFor="signup-city" className="block text-sm font-medium text-foreground mb-1.5">City</label>
        ...
        {isDropdownOpen && (
          <div className="absolute z-50 mt-1 w-full max-h-[200px] overflow-y-auto rounded-lg border border-border bg-background shadow-lg">
            {filteredCities.length === 0 ? (
              <div className="px-3.5 py-3 text-sm text-muted-foreground">No cities found</div>
            ) : (
              filteredCities.map((city) => (
                ...
                >
                  <span>{city.name}</span>
                  <span className="text-muted-foreground ml-2 text-xs">{city.state}</span>
                </button>
              ))
            )}
          </div>
      ...
      <div>
        <label htmlFor="signup-area" className="block text-sm font-medium text-foreground mb-1.5">Area / Locality <span className="text-muted-foreground font-normal ml-1">(optional)</span></label>
        <input
          id="signup-area"
          type="text"
          value={formData.area}
...
function StepHealth({
  ...
  const sliderPercent = ((formData.aqiThreshold - SLIDER_MIN) / (SLIDER_MAX - SLIDER_MIN)) * 100;
  ...
  return (
    <div className="space-y-6">
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">Health conditions</label>
        <p className="text-xs text-muted-foreground mb-3">Select any that apply — this personalizes your advisories.</p>
      ...
      <div>
        <label htmlFor="signup-threshold" className="block text-sm font-medium text-foreground mb-1">AQI alert threshold</label>
        <p className="text-xs text-muted-foreground mb-4">You&apos;ll be notified when AQI exceeds this value.</p>
        <div className="space-y-3">
          <input
            ...
            onChange={(e) => setFormData((prev) => ({ ...prev, aqiThreshold: Number(e.target.value) }))}
          ...
          <div className="flex justify-between text-[10px] font-medium text-muted-foreground px-0.5">
            <span>{formData.aqiThreshold} ({getThresholdLabel(formData.aqiThreshold)})</span>
