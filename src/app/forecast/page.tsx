...
function toChartData(
  ...
  return slots.map((s) => {
    ...
    const hourLabel = d.toLocaleTimeString("en-IN", {
      hour: "numeric",
      hour12: true,
    });
...
export default function ForecastPage() {
  ...
  const todayHourly = chartData
    .slice(0, 24)
    .map((d) => ({ hour: d.hour, label: d.time, aqi: d.aqi }));
  const forecastSlots =
    forecast?.forecast.map((s, i) => ({
      ...chartData[i],
      dominantPollutant: s.dominant_pollutant.toUpperCase(),
    })) ?? [];

  const tomorrowSlots = forecast?.forecast.slice(24) ?? [];
  const tomorrowAvg = tomorrowSlots.length
    ? Math.round(
        tomorrowSlots.reduce((s, d) => s + d.aqi, 0) / tomorrowSlots.length,
      )
    : 0;
  const tomorrowPeak = tomorrowSlots.length
    ? Math.max(...tomorrowSlots.map((d) => d.aqi))
    : 0;
  const tomorrowLow = tomorrowSlots.length
    ? Math.min(...tomorrowSlots.map((d) => d.aqi))
    : 0;
  ...
  return (
    <DashboardLayout>
        ...
        <div className="mb-6 flex items-center gap-3 rounded-xl border border-orange-500/30 bg-orange-500/5 px-4 py-3 text-sm text-orange-400">
          ...
          <div>
            ...
            <code className="rounded bg-orange-500/10 px-1.5 py-0.5 text-xs font-mono">
              cd ml-service &amp;&amp; uvicorn main:app --port 8081
            </code>
          </div>
        </div>
      )}
      ...
      <div className="text-center text-[10px] text-muted-foreground/40 pb-4">
        Forecast generated using ensemble ML model (SARIMA diurnal + LightGBM
        weather corrections) · CPCB NAQI methodology
      </div>
    </DashboardLayout>
  );
}
